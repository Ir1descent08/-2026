...
const char *class="CLASS 2441\n";
const char *StuNUM="CODE 524031910429\n";
const char *COM1="AT+CLASS";         //command 1
const char *COM2="AT+STUDENTCODE";   //command 2
...
void UARTStringPutNonBlocking(const char *cMessage)
{
	while(*cMessage!='\0')
	{
		if (UARTSpaceAvail(UART0_BASE))         //??FIFO???
...
void UART0_Handler(void)
{
  ...
  if(strcasecmp(COM1,msg)==0)            // ??strcasecmp(),??????
		UARTStringPutNonBlocking(class);   //????
	if(strcasecmp(COM2,msg)==0)
		UARTStringPutNonBlocking(StuNUM);          //????
